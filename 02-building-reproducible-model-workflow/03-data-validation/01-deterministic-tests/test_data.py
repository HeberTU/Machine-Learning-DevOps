...
def test_class_names(data):
    ...
    invalid_values = data[~data.genre.isin(known_classes)].genre.values

    assert data.genre.isin(known_classes).all(), f"Invalid values: {invalid_values}"
...
def test_column_ranges(data):
    ...
    for col_name, (minimum, maximum) in ranges.items():
        ...
        invalid_values = data[
            ~data[col_name].between(minimum, maximum)][col_name].\
            unique()

        msg = f"{col_name}: ({minimum}, {maximum})-{invalid_values}"

        result = data[col_name].dropna().between(minimum, maximum).all()
